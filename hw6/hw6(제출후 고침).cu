...
__global__ void maxpool(float *input, float *output, const int input_size, const int filter_size) {
    ...
    int tmp_index = (input_size*filter_size*row)+(filter_size*col)

	for(int i = 0; i < filter_size; i++){
		for(int j = 0; j < filter_size; j++){
			tmp = input[tmp_index+(input_size*j)+i];
			if(Max<tmp)
				Max = tmp;
		}
	}
	...
	if(col < output_size && row < output_size)
		output[output_size*row+col] = Max;
...
__global__ void gemm(float *a, float *b, float *c, const float alpha, const float beta, float *output, const int input_size){
    ...
    int sqr_input_size = input_size * input_size
	int a_index;
	int b_index;
    int output_index = row*input_size + col;
    ...
    for(int p = 0; p < input_size/TILE_WIDTH+1 ;p++){
		...
		if(a_index < sqr_input_size )
			s_a[ty][tx] = a[a_index];
		else
			s_a[ty][tx] = 0.0;

		if(b_index < sqr_input_size )
			s_b[ty][tx] = b[b_index];
		else
			s_b[ty][tx] = 0.0;
...
//		s_b[ty][tx] = b[(ty+p*TILE_WIDTH)*input_size + col];
	...
	if(row < input_size && col < input_size)
		output[output_index] = (alpha * result) + (beta * c[output_index]);
	//__syncthreads(); 
}
