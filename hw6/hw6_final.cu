...
#define TILE_WIDTH 5/* set TILE_WIDTH 16 for the evaluation! */
...
__global__ void gemm(float *a, float *b, float *c, const float alpha, const float beta, float *output, const int input_size){
	...
	axx = row*input_size + p*TILE_WIDTH +tx;
	bxx = (ty + p*TILE_WIDTH)*input_size + col;

	if(axx < input_size * input_size )
		s_a[ty][tx] = a[axx];
	else
		s_a[ty][tx] = 0.0;

	if(bxx < input_size*input_size )
		s_b[ty][tx] = b[bxx];
	else
		s_b[ty][tx] = 0.0;
